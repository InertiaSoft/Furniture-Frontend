  ...
  return (
    <>
      {error ? (
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-3">
          {data?.map((category, i) => (
            <li className="group p-4" key={i + 1}>
              <div
                onClick={() =>
                  router.push(
                ...
                className="flex w-full h-full border border-black-100 shadow-sm bg-white p-4 cursor-pointer transition duration-200 ease-linear transform group-hover:shadow-lg"
              >
